...
async function processCleanSpriteSheet(inputJpgPath, outputWebpPath, isDirection = false) {
  ...
  const colSplits = [0, 345, 705, 1080];
  ...
  for (let r = 0; r < 3; r++) {
    for (let c = 0; c < 3; c++) {
      // Natural direct mapping (no column swap)
      const srcCol = c;
      const srcRow = r;
      ...
      function isWhite(x, y) {
        ...
        return regionData[idx] > 220 && regionData[idx + 1] > 220 && regionData[idx + 2] > 220;
      }
      ...
      // Anchor character: centered horizontally, anchored 20px from bottom of cell
      ...
      console.log(`Cell [${r},${c}]: ${scaledW}x${scaledH}, placed at cell pos (${pasteX}, ${pasteY})`);
  ...
  const outBuffer = await sharp(outRgba, { raw: { width: W, height: H, channels: 4 } })
    .webp({ quality: 95, alphaQuality: 100 })
    .toBuffer();

  fs.writeFileSync(outputWebpPath, outBuffer);
  console.log(`Successfully written via buffer: ${outputWebpPath}`);
}

async function main() {
  const dirJpg = 'C:\\Users\\Administrator\\.gemini\\antigravity-ide\\brain\\ae324bbc-f55c-4500-8cbb-40a07a034b54\\cheobingo_mim_dir_1790130961862.jpg';
  const reactJpg = 'C:\\Users\\Administrator\\.gemini\\antigravity-ide\\brain\\ae324bbc-f55c-4500-8cbb-40a07a034b54\\cheobingo_mim_react_1790131126094.jpg';

  const outDir = path.join(__dirname, 'public', 'mascots');

  console.log('--- Processing Directions (Cuoi Mim, Proper Orientation) ---');
  await processCleanSpriteSheet(dirJpg, path.join(outDir, 'cheobingo-directions.webp'), true);

  console.log('--- Processing Reactions (Cuoi Mim, Sweet & Friendly) ---');
