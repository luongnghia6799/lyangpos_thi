...
async function processCleanSpriteSheet(inputJpgPath, outputWebpPath, isDirection = false) {
  ...
  // Exact row and col split boundaries (count=0 pure white lines)
  ...
  for (let r = 0; r < 3; r++) {
    for (let c = 0; c < 3; c++) {
      // If directions: Column 0 and Column 2 are swapped so that:
      // Target col 0 (left) gets source col 2 (which looks left)
      // Target col 2 (right) gets source col 0 (which looks right)
      ...
      console.log(`Target Cell [${r},${c}] (from src [${srcRow},${srcCol}]): ${scaledW}x${scaledH}, placed at cell pos (${pasteX}, ${pasteY})`);
...
async function main() {
  ...
  console.log('--- Processing Directions v3 (Friendly & Swapped Left/Right) ---');
  await processCleanSpriteSheet(dirJpg, path.join(outDir, 'cheobingo-directions.webp'), true);

  console.log('--- Processing Reactions v3 (Friendly & Sweet) ---');
