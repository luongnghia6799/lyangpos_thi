    ...
    useEffect(() => {
        const fetchFontsAndTemplates = async () => {
            try {
                ...
                // Preload any Google Fonts set on print templates or server settings
                if (settingsRes.data) {
                    if (settingsRes.data.invoice_font_family) {
                        loadGoogleFont(settingsRes.data.invoice_font_family);
                    }
                }
