...
import { MASCOT_LIST, DEFAULT_MASCOT_CONFIG, playPopSound } from '../lib/mascots';
import { X, Sparkles, Volume2, VolumeX, Lock, Unlock, RotateCcw, Eye, MessageSquare, Check, Sliders } from 'lucide-react';
...
const MascotSettingsModal = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100000] flex items-center justify-center bg-black/60 backdrop-blur-sm animate-in fade-in duration-200 p-4">
      <div
        className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl shadow-2xl w-full max-w-xl max-h-[90vh] flex flex-col overflow-hidden text-slate-800 dark:text-slate-100 animate-in zoom-in-95 duration-200"
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400">
                Tương tác, theo dõi chuột & biểu cảm sinh động
              </p>
            </div>
          </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              <label className="text-sm font-bold text-slate-800 dark:text-slate-200 flex items-center gap-2">
                <span>Chọn Nhân Vật ({MASCOT_LIST.length})</span>
              </label>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3">
              {MASCOT_LIST.map((char) => {
                const isSelected = config.characterId === char.id;
                return (
                  <button
                    key={char.id}
                    onClick={() => handleSelectCharacter(char.id)}
                    className={`relative flex flex-col items-center p-3 rounded-2xl border transition-all text-center cursor-pointer group ${
                      isSelected
                  ...
                  >
                    {isSelected && (
                      <div className="absolute top-2 right-2 w-5 h-5 rounded-full bg-amber-500 text-white flex items-center justify-center shadow-sm">
                    ...
                    <div
                      ...
                      className="rounded-full mb-2 group-hover:scale-110 transition-transform duration-200"
                    />
