    ...
    const handleSave = async () => {
        ...
        try {
            ...
            try {
                ...
                if (order.partner_id) {
                    bc.postMessage({ type: 'PARTNER_UPDATED', partnerId: order.partner_id });
                }
                bc.close();
                window.dispatchEvent(new CustomEvent('pos_data_sync', { detail: { type: 'ORDER_SAVED', orderId: order.id } }));
                if (order.partner_id) {
                    window.dispatchEvent(new CustomEvent('pos_data_sync', { detail: { type: 'PARTNER_UPDATED', partnerId: order.partner_id } }));
                }
            } catch (syncErr) {
                console.error("Broadcast sync error in OrderEditPopup:", syncErr);
            }
            onSave();
        } catch (err) {
