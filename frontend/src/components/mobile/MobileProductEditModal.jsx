    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const payload = {
                ...
                multiplier: formData.multiplier !== '' && formData.multiplier !== null && formData.multiplier !== undefined ? Number(formData.multiplier) : 1,
                cost_price: formData.cost_price !== '' && formData.cost_price !== null && formData.cost_price !== undefined ? Number(formData.cost_price) : 0,
                sale_price: formData.sale_price !== '' && formData.sale_price !== null && formData.sale_price !== undefined ? Number(formData.sale_price) : 0,
                stock: formData.stock !== '' && formData.stock !== null && formData.stock !== undefined ? Number(formData.stock) : 0,
            };
            let res;
            if (product?.id) {
                res = await axios.put(`/api/products/${product.id}`, payload);
            } else {
