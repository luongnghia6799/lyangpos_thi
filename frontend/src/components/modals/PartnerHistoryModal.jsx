...
import PrintTemplate from '../panels/PrintTemplate';
                            ...
                            <div className="p-6 border-b dark:border-slate-800 bg-transparent/50 dark:bg-slate-800/50 flex justify-between items-center">
                                <div>
                                    <h3 className="text-2xl font-black text-emerald-600 dark:text-emerald-400 uppercase tracking-tighter flex items-center gap-3">
                                        <FileText size={28} />
                                        Lịch sử giao dịch: #{currentPartner?.id || partner?.id} {currentPartner?.name || partner?.name || 'Khách hàng'}
                                    </h3>
                                    <p className="text-sm font-bold text-gray-500 uppercase mt-1 flex items-center gap-4">
                                        <span>SĐT: {currentPartner?.phone || partner?.phone || 'N/A'}</span>
                                        <span>| Nợ hiện tại: <span className={cn("text-lg font-black", (currentPartner?.debt_balance ?? partner?.debt_balance ?? 0) > 0 ? "text-blue-600" : (currentPartner?.debt_balance ?? partner?.debt_balance ?? 0) < 0 ? "text-red-600" : "text-gray-400")}>{formatNumber(Math.abs(currentPartner?.debt_balance ?? partner?.debt_balance ?? 0))} {(currentPartner?.debt_balance ?? partner?.debt_balance ?? 0) > 0 ? '(Nợ mình)' : (currentPartner?.debt_balance ?? partner?.debt_balance ?? 0) < 0 ? '(Mình nợ)' : ''}</span></span>
                                    </p>
