...
class TTSEngine {
    ...
    getBaseUrl() {
        ...
        let baseUrl = `http://localhost:${defaultPort}`;
        if (typeof window !== 'undefined' && window.location && window.location.hostname && window.location.hostname !== 'localhost' && window.location.hostname !== '127.0.0.1' && !window.location.hostname.includes('tauri')) {
            baseUrl = `http://${window.location.hostname}:${defaultPort}`;
        }
        return baseUrl;
    }
    ...
    getAudioUrls(word) {
        ...
        const localApiUrl = `${baseUrl.replace(/\/+$/, '')}/api/tts?text=${encodedText}&voice=${voiceParam}&rate=${rate}&pitch=${encodeURIComponent(pitch)}`;
