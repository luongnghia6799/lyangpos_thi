...
// Note: Speed rate is already natively synthesized into the audio file by Edge-TTS SSML,
// so Web Audio playbackRate must be 1.0 to avoid double-speed distortion and clipped endings.
const playAudioBuffer = (audioBuffer, rate = 1.0) => {
  try {
...
const getCacheName = (voiceParam = '') => {
  if (voiceParam && typeof voiceParam === 'string') {
    if (voiceParam.includes('male') && !voiceParam.includes('female')) return 'lyang-tts-audio-cache-male-v2';
    if (voiceParam.includes('female')) return 'lyang-tts-audio-cache-female-v2';
  }
  const current = (typeof localStorage !== 'undefined' && (localStorage.getItem('pos_selected_voice') || localStorage.getItem('pos_tts_mode'))) || '';
  return (current === 'edge-vi-male' || current === 'male') ? 'lyang-tts-audio-cache-male-v2' : 'lyang-tts-audio-cache-female-v2';
};
    ...
    if (priority === 'low') {
      if (typeof caches !== 'undefined') {
        try {
          ...
          console.warn("Low-priority Cache API write failed, falling back to direct get:", err);
        }
      }
      // If Cache API is not available (e.g., http without secure context in WebView), still trigger backend synthesis & disk cache
      await axios.get(audioUrl, { responseType: 'arraybuffer' }).catch(() => null);
      return null;
    }
...
const trimAudioBuffer = (ctx, buffer) => {
  try {
    ...
    // -50dB threshold to detect gentle word endings (t, c, p, ch, n, nh)
    ...
    outerStart: for (let i = 0; i < length; i++) {
      for (let ch = 0; ch < numChannels; ch++) {
        if (Math.abs(buffer.getChannelData(ch)[i]) > threshold) {
          start = Math.max(0, i - Math.floor(sampleRate * 0.04)); // keep 40ms attack margin
          break outerStart;
        }
      }
    }
    ...
    outerEnd: for (let i = length - 1; i >= 0; i--) {
      for (let ch = 0; ch < numChannels; ch++) {
        if (Math.abs(buffer.getChannelData(ch)[i]) > threshold) {
          end = Math.min(length - 1, i + Math.floor(sampleRate * 0.08)); // keep 80ms release margin so word tail is complete
          break outerEnd;
        }
      }
    }
...
const getDynamicBaseUrl = () => {
  const defaultPort = (typeof import.meta !== 'undefined' && import.meta.env && import.meta.env.VITE_BACKEND_PORT) || ((typeof import.meta !== 'undefined' && import.meta.env && import.meta.env.DEV) ? '3579' : '3579');
...
export const cancelPrecacheCommonTTS = () => {
  activePrecacheSessionId++;
  if (typeof window !== 'undefined') {
    window.ttsPrecacheProgress = { completed: 0, total: 0, active: false, cancelled: true };
    window.dispatchEvent(new CustomEvent('tts-precache-progress', { 
      detail: { completed: 0, total: 0, active: false, cancelled: true } 
    }));
  }
};

export const clearTTSAudioCache = async () => {
  try {
    cancelPrecacheCommonTTS();
    
    // 1. Clear in-memory AudioBuffer cache
    Object.keys(ttsAudioCache).forEach(k => delete ttsAudioCache[k]);
    ...
    if (typeof caches !== 'undefined') {
      try {
        ...
        for (const name of cacheNames) {
          if (name.includes('tts') || name.includes('lyang-tts')) {
    ...
    // 3. Clear Backend disk cache
    const baseUrl = getDynamicBaseUrl();
    await axios.post(`${baseUrl.replace(/\/+$/, '')}/api/tts/clear`).catch(() => null);

    if (typeof window !== 'undefined') {
      window.dispatchEvent(new CustomEvent('pos-notify', {
        detail: { message: "Đã xóa toàn bộ bộ nhớ đệm âm thanh thành công!", type: 'success' }
      }));
    }
    return true;
  } catch (err) {
    console.error("clearTTSAudioCache error:", err);
    return false;
  }
};
...
export const precacheCommonTTS = (products = [], options = {}) => {
  try {
    ...
    // Voices to precache: active voice first, then secondary voice
    ...
    const dispatchProgress = (completed, total, active, currentText = "") => {
      if (typeof window !== 'undefined') {
        window.ttsPrecacheProgress = { completed, total, active, currentText };
        window.dispatchEvent(new CustomEvent('tts-precache-progress', { 
          detail: { completed, total, active, currentText } 
        }));
      }
    };

    // 3. Queue audio requests with concurrency limit
    let index = 0;
    ...
    const loadNext = async () => {
      if (currentSessionId !== activePrecacheSessionId) {
        return; // Session was cancelled or replaced
      }
      if (index >= totalCount) {
        if (completedCount >= totalCount) {
          ...
          if (typeof window !== 'undefined') {
            window.dispatchEvent(new CustomEvent('pos-notify', {
              detail: { message: `Đã hoàn tất tải trước ${totalCount} file âm thanh Offline!`, type: 'success' }
            }));
          }
        }
        return;
      ...
      if (currentSessionId !== activePrecacheSessionId) return;

      completedCount++;
      dispatchProgress(completedCount, totalCount, true, text);
    ...
    if (cached) {
      if (cached instanceof Promise) {
        return cached.then(buffer => {
          if (buffer) {
            return playAudioBuffer(buffer, 1.0);
          } else {
        ...
        return playAudioBuffer(cached, 1.0);
      }
    } else {
