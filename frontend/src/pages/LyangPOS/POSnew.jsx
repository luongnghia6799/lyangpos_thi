                  ...
                  Xr(!0);
                    ...
                    scale: 0.98
                      ...
                      <button onClick={() => {
                        ...
                        const t = !showLastPurchaseBadge;
                        setShowLastPurchaseBadge(t), localStorage.setItem("pos_show_last_purchase_badge", t ? "true" : "false");
                        try {
                          const a = new BroadcastChannel("pos_data_sync");
                          a.postMessage({
                            type: "UI_SETTING_UPDATED",
                            key: "pos_show_last_purchase_badge",
                            value: t ? "true" : "false"
                          }), a.close();
                        } catch {}
                      }} className="flex items-center justify-between px-3 py-2.5 text-xs font-black text-slate-700 dark:text-slate-200 hover:bg-emerald-500/10 hover:text-primary dark:hover:text-emerald-400 rounded-2xl transition-all border-t border-slate-100 dark:border-slate-800/80 pt-2.5 mt-0.5 group/menu-item w-full text-left"><div className="flex items-center gap-3"><div className="w-8 h-8 rounded-xl bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 flex items-center justify-center group-hover/menu-item:scale-110 transition-transform"><Ao size={16} strokeWidth={2.5} /></div><div className="flex flex-col text-left"><span className="uppercase tracking-tight text-[11px]">Badge ngày mua gần nhất</span><span className="text-[9px] font-bold text-slate-400 lowercase tracking-normal">{showLastPurchaseBadge ? "bật: hiện ngày mua của khách" : "tắt: ẩn badge ngày mua"}</span></div></div><div className={c("w-9 h-5 rounded-full p-0.5 transition-colors duration-200 ease-in-out shrink-0 flex items-center border", showLastPurchaseBadge ? "bg-indigo-600 border-indigo-600 justify-end" : "bg-slate-200 dark:bg-slate-700 border-slate-300 dark:border-slate-600 justify-start")}><div className="w-4 h-4 rounded-full bg-white shadow-sm" /></div></button><button onClick={() => {
                        Ct(!1), Ci();
                ...
                return <div style={{
                  ...
                  height: `${Jr}px`
                    ...
                    Yr(105), localStorage.setItem("pos_bottom_summary_height", "105");
                      ...
                      }} className={c("flex-1 min-h-[34px] relative overflow-hidden p-1 px-2.5 rounded-xl border cursor-pointer transition-all duration-300 hover:scale-[1.01] active:scale-[0.99] min-w-0 flex flex-col justify-between group/debt-card select-none", s > 0 ? "bg-gradient-to-br from-[#7f1d1d] via-[#991b1b] to-[#881337] dark:from-[#4c0519] dark:via-[#881337] dark:to-[#4c0519] text-white border-rose-400/50 shadow-[0_0_18px_rgba(244,63,94,0.35)] hover:shadow-[0_0_25px_rgba(244,63,94,0.55)]" : s < 0 ? "bg-gradient-to-br from-[#14532d] via-[#166534] to-[#14532d] text-white border-emerald-400/50 shadow-[0_0_18px_rgba(16,185,129,0.35)] hover:shadow-[0_0_25px_rgba(16,185,129,0.55)]" : "bg-gradient-to-br from-[#1b4332] via-[#2d6a4f] to-[#1b4332] dark:from-[#0d281e] dark:via-[#164230] dark:to-[#0d281e] text-white border-emerald-400/40 shadow-[0_0_18px_rgba(16,185,129,0.3)] hover:shadow-[0_0_25px_rgba(16,185,129,0.45)]")} style={s <= 0 && cartColorConfig.accentColor && cartColorConfig.accentColor !== 'default' ? { background: `linear-gradient(135deg, ${cartColorConfig.accentColor}, ${cartColorConfig.borderColor !== 'default' ? cartColorConfig.borderColor : cartColorConfig.accentColor}dd)`, borderColor: cartColorConfig.borderColor !== 'default' ? cartColorConfig.borderColor : `${cartColorConfig.accentColor}80`, boxShadow: cartColorConfig.enableGlow !== false ? `0 0 20px ${cartColorConfig.accentColor}35` : undefined } : undefined} title={a ? `Xem lịch sử nợ của ${r}` : "Chưa chọn đối tác"}><div className="absolute inset-0 bg-gradient-to-b from-white/15 via-transparent to-transparent pointer-events-none" /><div className="absolute -right-1.5 -bottom-2 opacity-15 text-white pointer-events-none -rotate-6 transition-transform group-hover/debt-card:scale-110 select-none"><Va size={42} strokeWidth={1.5} /></div><div className="flex items-center justify-between w-full relative z-10 pt-0.5"><span className={c("text-[9px] font-black uppercase tracking-wider leading-normal", s > 0 ? "text-rose-200" : "text-emerald-200")}>DƯ NỢ</span>{a && s !== 0 && <x.span initial={{
                            opacity: 0,
                            scale: 0.8
                          }} animate={{
                            opacity: 1,
