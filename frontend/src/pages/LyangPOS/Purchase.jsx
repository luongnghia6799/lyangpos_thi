...
import { Search, Plus, Minus, Trash2, Save, X, Printer, User, Users, Phone, FileText, ShoppingCart, Activity, History, Package, TrendingDown, TrendingUp, AlertTriangle, Truck, Pause, RotateCcw, Sprout, Wheat, Droplets, Coins, Leaf, Warehouse, Eye, Keyboard, ChevronLeft, ChevronRight, Loader2, Clock, MapPin, Wallet, Bot, Sparkles, Camera, Upload, Check } from 'lucide-react';
...
export default function Purchase() {
    ...
    const findBestMatch = (scannedName, productsList) => {
        ...
        const cleanScanned = removeAccents(scannedName.toLowerCase().trim());
        const wordsScanned = cleanScanned.split(/[\s\-_,./+*()]+/).filter(w => w.length > 0);
        
        let bestMatch = null;
        let maxScore = 0;
        
        for (const p of productsList) {
            const cleanProd = removeAccents(p.name.toLowerCase().trim());
            
            // 1. Exact match
            if (cleanScanned === cleanProd) {
                return p;
            }
            
            // 2. Substring matching
            if (cleanScanned.includes(cleanProd) || cleanProd.includes(cleanScanned)) {
                const score = Math.min(cleanProd.length, cleanScanned.length) / Math.max(cleanProd.length, cleanScanned.length) + 0.5;
                if (score > maxScore) {
                    maxScore = score;
                    bestMatch = p;
                }
            ...
            // 3. Length-weighted word overlap with Brand Match Bonus
            const wordsProd = cleanProd.split(/[\s\-_,./+*()]+/).filter(w => w.length > 0);
            
            // Exclude single character helper words/separators from matching unless they are digits
            const commonWords = wordsScanned.filter(w => wordsProd.includes(w) && (w.length > 1 || /^\d+$/.test(w)));
            
            if (commonWords.length > 0) {
                const matchedLength = commonWords.reduce((sum, w) => sum + w.length, 0);
                const totalLength = Math.max(cleanProd.length, cleanScanned.length);
                let score = matchedLength / totalLength;
                
                // Brand Match Bonus: If first word of scanned name matches a word in system product
                if (wordsScanned[0] && wordsProd.includes(wordsScanned[0])) {
                    score += 0.6;
                }
    ...
    const getTopMatches = (scannedName, productsList, limit = 3) => {
        ...
        const scored = productsList.map(p => {
            ...
            if (cleanScanned === cleanProd) {
                score = 1.5;
            } else if (cleanScanned.includes(cleanProd) || cleanProd.includes(cleanScanned)) {
                score = Math.min(cleanProd.length, cleanScanned.length) / Math.max(cleanProd.length, cleanScanned.length) + 0.5;
            } else {
                const wordsProd = cleanProd.split(/[\s\-_,./+*()]+/).filter(w => w.length > 0);
                const commonWords = wordsScanned.filter(w => wordsProd.includes(w) && (w.length > 1 || /^\d+$/.test(w)));
                ...
                if (commonWords.length > 0) {
                    const matchedLength = commonWords.reduce((sum, w) => sum + w.length, 0);
                    score = matchedLength / Math.max(cleanProd.length, cleanScanned.length);
  ...
  // HeavyClock handles time
    ...
    useEffect(() => {
        const handleKeyDown = (e) => {
            ...
            // F1: Focus Supplier
            if (e.keyCode === 112 || key === 'F1') {
                e.preventDefault();
                partnerInputRef.current?.focus();
                partnerInputRef.current?.select();
            }
            // F3: Focus Search Partner (Legacy/Alternative)
            else if (e.keyCode === 114 || key === 'F3') {
                e.preventDefault();
                partnerInputRef.current?.focus();
                partnerInputRef.current?.select();
            }
            ...
            <div className="flex-1 flex flex-col overflow-hidden no-print">
                ...
                <div className="p-4 flex gap-6 items-center print:hidden transition-colors relative border-[#d4a574]/20 z-[2500]">
                    ...
                    <div className="flex items-center gap-3 mr-4 relative z-10">
                        <h1 className="text-2xl font-black text-primary dark:text-[#d4a574] uppercase tracking-tighter flex items-center gap-3 py-1">
                            Kho Hàng
                            <AnimatePresence mode="popLayout" initial={false}>
                                ...
                                >
                                    <div className="flex items-center bg-transparent px-3 py-1.5 rounded-xl border border-black/10 dark:border-white/10">
                                        <div className={cn("w-2 h-2 rounded-full mr-3", editOrderId ? "bg-amber-500" : "bg-emerald-500")} />
                                        <div className="flex flex-col">
                                            <span className="text-base font-black font-mono text-amber-700 dark:text-[#d4a574] tracking-tight leading-none">
                                                #{editingOriginalOrder?.display_id || editOrderId || 'MỚI'}
                                            </span>
                                            {editingOriginalOrder?.date && (
                                                <span className="text-[10px] font-bold text-slate-500 dark:text-[#d4a574]/80 mt-1 tabular-nums whitespace-nowrap">
                                                    {formatDate(editingOriginalOrder.date)}
                                                </span>
                                            )}
                                        </div>
                                    </div>
                    ...
                    <div className="flex gap-2 items-center">
                        <button
                            ...
                            className="w-11 h-11 flex items-center justify-center bg-[#4a7c59] hover:bg-[#3d654a] text-white rounded-full transition-all border border-white/20"
                            title="Xem trước in"
                        >
                            <Eye size={20} strokeWidth={2.5} />
                        </button>
                    </div>
                    <div className="flex-1 flex gap-4 relative z-[2100]">

                        <div className="flex-1 max-w-[800px] relative z-50 group/partner" onBlur={() => setTimeout(() => setIsPartnerDropdownOpen(false), 200)}>
                            <div
                            ...
                            >
                                {/* Search Icon / Avatar */}
                                <div className="absolute left-1.5 top-1/2 -translate-y-1/2 w-10 h-10 flex items-center justify-center transition-all group-focus-within/partner:scale-110">
                                    <User
                                ...
                                <input
                                    type="text"
                                    className="w-full pl-14 pr-12 py-3.5 bg-transparent dark:bg-transparent border border-black/10 focus:border-[#d4a574]/20 dark:focus:border-[#d4a574]/30 rounded-2xl outline-none font-bold text-[15px] dark:text-white leading-normal transition-all shadow-none focus:bg-transparent dark:focus:bg-transparent"
                                    ref={partnerInputRef}
                                    placeholder="Tìm kiếm nhà cung cấp (F3)..."
                                    ...
                                    onKeyDown={(e) => {
                                        if (e.key === 'ArrowDown') {
                                            ...
                                            setActiveIndex(prev => {
                                                ...
                                                if (container) {
                                                    const item = next === 0 ? container.children[0] : container.children[1]?.children[next - 1];
                                ...
                                <div className="absolute right-4 top-1/2 -translate-y-1/2 flex items-center gap-2">
                                    {selectedPartner ? (
                                        <button
                                            onClick={(e) => { e.stopPropagation(); setSelectedPartner(null); setPartnerSearch(''); }}
                                            className="w-7 h-7 flex items-center justify-center rounded-full bg-rose-50 dark:bg-rose-500/10 text-rose-500 hover:bg-rose-100 transition-colors shadow-sm"
                                        >
                                            <X size={14} strokeWidth={3} />
                                        </button>
                                    ) : (
                                        <span className="text-[10px] font-black opacity-20 border border-current px-1 rounded uppercase tracking-tighter">F3</span>
                                    )}
                                </div>
                            </div>
                            ...
                            <AnimatePresence>
                                {isPartnerDropdownOpen && !selectedPartner && (
                                    ...
                                    >
                                        <div
                                            className={cn(
                                                "dropdown-item flex items-center gap-4 group/item py-3",
                                                activeIndex === 0 && "active"
                                            )}
                                            onMouseEnter={() => setActiveIndex(0)}
                                            onClick={() => { setSelectedPartner(null); setPartnerSearch(''); setIsPartnerDropdownOpen(false); }}
                                        >
                                            <div className={cn("w-10 h-10 rounded-full flex items-center justify-center transition-all", activeIndex === 0 ? "bg-white/20 text-white scale-110" : "bg-emerald-100 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400 group-hover/item:bg-emerald-200")}>
                                                <User size={20} strokeWidth={2.5} />
                                            </div>
                                            <div>
                                                <div className={cn("font-black uppercase tracking-tight transition-all duration-300", activeIndex === 0 ? "text-white" : "text-emerald-900 dark:text-emerald-50")} style={{
                                                    paddingLeft: activeIndex === 0 ? '12px' : '0px'
                                                }}>Nhà cung cấp vãng lai</div>
                                                <div className={cn("text-[10px] font-bold uppercase tracking-widest opacity-60", activeIndex === 0 ? "text-white/70" : "text-emerald-500")}>Thanh toán ngay</div>
                                            </div>
                                        </div>
                                            ...
                                            {filteredPartners.map((p, idx) => (
                                                ...
                                                >
                                                    <div className="flex items-center gap-4 group/item">
                                                        <div className={cn("w-10 h-10 rounded-full flex items-center justify-center transition-all", (activeIndex === idx + 1) ? "bg-white/20 text-white scale-110" : "bg-emerald-100 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400 group-hover/item:bg-emerald-200")}>
                                                            <User size={20} strokeWidth={2.5} />
                                                        </div>
                                                        <div>
                                                            <div className={cn("font-black uppercase tracking-tight flex items-center gap-2", (activeIndex === idx + 1) ? "text-white" : "text-emerald-900 dark:text-emerald-50")}>
                                                                <span className={cn(
                                                                    "px-2 py-0.5 rounded-lg text-[10px] font-black shadow-sm",
                                                                    (activeIndex === idx + 1) ? "bg-white !text-emerald-700" : "bg-emerald-50 text-emerald-600 border border-emerald-100"
                                                                )}>
                                                                    #{p.id}
                                                                </span>
                                                                <span className="transition-all duration-300" style={{
                                                                    paddingLeft: activeIndex === idx + 1 ? '12px' : '0px'
                                                                }}>{p.name}</span>
                                                            </div>
                                                            <div className={cn("text-[10px] font-bold flex items-center gap-3", (activeIndex === idx + 1) ? "text-white/70" : "text-emerald-500/80")}>
                                                                <span className="flex items-center gap-1"><Phone size={10} strokeWidth={3} /> {p.phone || '---'}</span>
                                                                <span className="w-1 h-1 rounded-full bg-emerald-300" />
                                                                <span className="flex items-center gap-1 uppercase tracking-tighter">
                                                                    {p.is_customer && p.is_supplier ? "Khách & NCC" : "Nhà cung cấp"}
                                                                </span>
                                                            </div>
                                                        </div>
                                                    </div>
                                        ...
                                        {partnerSearch && (
                                            <div
                                                className="p-4 bg-primary text-white cursor-pointer font-black uppercase text-xs flex items-center gap-2 hover:brightness-110 shadow-inner transition-all border-t border-white/10"
                                                onClick={() => {
                                            ...
                                            >
                                                ...
                                                {filteredPartners.length === 0 ? "Thêm nhà cung cấp mới: " : "Vẫn tạo mới nhà cung cấp: "}
                                                "{partnerSearch}"
                                            </div>
                                        )}
                                    </m.div>
                        ...
                        <div className="flex gap-2 items-center">
                            ...
                            <m.button
                                ...
                                className="relative w-11 h-11 bg-[#f4c430] hover:bg-[#eab308] text-white rounded-full transition-all flex items-center justify-center border border-white/20"
                                title="Đơn nhập tạm"
                            >
                                <Pause size={20} />
                                {heldPurchases.length > 0 && (
                                    <span className="absolute -top-1.5 -right-1.5 bg-rose-600 text-white text-[9px] w-5 h-5 rounded-full flex items-center justify-center font-black border border-white">
                                        {heldPurchases.length}
                                    </span>
                                )}
                            </m.button>

                            <m.div className="flex items-center rounded-full border border-black/10 dark:border-white/10 bg-transparent p-0.5 transition-colors">
                                <m.button
                                    onClick={() => navigateHistory('prev')}
                                    whileTap={{ scale: 0.9 }}
                                    className="w-8 h-8 flex items-center justify-center transition-colors rounded-full bg-amber-500 text-black hover:bg-amber-600"
                                    title="Đơn trước"
                                >
                                ...
                                >
                                    <span className={cn("text-[12px] font-black uppercase tracking-tighter", historyStep === 0 ? "text-primary" : "text-amber-500 dark:text-amber-400")}>
                                ...
                                <m.button
                                    ...
                                    className={cn(
                                        ...
                                        historyStep === 0 ? "text-slate-200 dark:text-slate-800" : "bg-amber-500 text-black hover:bg-amber-600"
                                    )}
                                >
                            ...
                            <m.button
                                ...
                                className="relative w-11 h-11 bg-transparent dark:bg-slate-950/90 backdrop-blur-sm text-emerald-900/60 hover:text-emerald-700 hover:bg-emerald-50 dark:hover:bg-emerald-900/20 rounded-full transition-all flex items-center justify-center border border-emerald-100 dark:border-emerald-900/50 group"
                                title="Chế độ nhập hàng sỉ/lẻ"
                            >
                                {posMode === 'Wholesale' ? <Users size={20} strokeWidth={2.5} /> : <User size={20} strokeWidth={2.5} />}
                                <div className={cn(
                                    "absolute bottom-2 right-2 w-2 h-2 rounded-full border border-white",
                                    posMode === 'Wholesale' ? "bg-emerald-500" : "bg-emerald-400"
                                )} />
                            </m.button>
                            ...
                            <m.button
                                ...
                                className="relative w-11 h-11 bg-[#4a7c59] hover:bg-[#3d654a] text-white rounded-full transition-all flex items-center justify-center border border-white/20 shadow-md group"
                                title="Quét hóa đơn nhập hàng bằng AI"
                            >
                                <Bot size={20} strokeWidth={2} />
                                <div className="absolute -top-0.5 -right-0.5 w-4 h-4 bg-rose-500 rounded-full border-2 border-white dark:border-slate-900 flex items-center justify-center">
                            ...
                            {selectedPartner && (
                                <m.button
                                    ...
                                    className="w-11 h-11 bg-[#4a7c59] hover:bg-[#3d654a] text-white rounded-full transition-all flex items-center justify-center border border-white/20"
                            ...
                            {selectedPartner && hasConsignmentOrders && (
                                <m.button
                                    whileTap={{ scale: 0.95 }}
                                    onClick={() => setIsConsignmentPanelOpen(true)}
                                    className="w-11 h-11 bg-[#b25e16] hover:bg-[#964e10] text-white rounded-full transition-all flex items-center justify-center border border-white/20"
                                    title="Theo dõi hàng gửi kho"
                                >
                ...
                <div className="flex-1 flex gap-3 p-4 pt-0 pb-4 print:hidden min-h-0 relative">
                    ...
                    >
                        <div className="flex-1 overflow-hidden relative pos-card shadow-md rounded-[2.7rem] border border-[#8b6f47]/30 dark:border-emerald-500/25">
                            <div className="w-full h-full rounded-[2.5rem] overflow-hidden relative bg-transparent">
                                ...
                                <div className="absolute inset-0 overflow-auto">
                                    <div className="min-w-[1000px] transition-colors relative group/decoration pb-[400px]">
                                        {/* Background Decoration Layer */}
                                        ...
                                        <table className="w-full text-left border-collapse">
                                            <thead className="bg-transparent sticky top-0 z-[100] print:hidden">
                                                <tr className="border-none">
                                                    <th rowSpan={2} className="py-1.5 px-2 w-14 text-center align-middle font-black uppercase text-[9px] tracking-widest text-slate-400 border-r border-white/10 dark:border-slate-800/20 whitespace-nowrap">Stt</th>
                                                    <th className="px-4 py-1 align-middle text-center min-w-[450px] whitespace-nowrap">
                                                        <div className="flex items-center justify-center gap-3 h-full min-h-[36px]">
                                                            <span className="font-black uppercase tracking-widest text-[9px] text-slate-500/80 dark:text-slate-400">Tên Sản Phẩm</span>
                                                        </div>
                                                    </th>
                                                    <th rowSpan={2} className="py-1.5 px-3 w-28 text-center align-middle font-black uppercase text-[9px] tracking-widest text-slate-400 border-x border-white/10 dark:border-slate-800/20 whitespace-nowrap">ĐVT</th>
                                                    <th className="py-1.5 px-3 w-36 text-center font-black uppercase text-[9px] tracking-widest text-slate-400 border-r border-white/10 dark:border-slate-800/20 whitespace-nowrap">SL Quy đổi</th>
                                                    <th className="py-1.5 px-3 w-20 text-center font-black uppercase text-[9px] tracking-widest text-slate-400 border-r border-white/10 dark:border-slate-800/20 whitespace-nowrap">Số lượng</th>
                                                    <th rowSpan={2} className="py-1.5 px-3 w-36 text-center align-middle font-black uppercase text-[9px] tracking-widest text-slate-400 border-r border-white/10 dark:border-slate-800/20 whitespace-nowrap">Giá nhập</th>
                                                    <th rowSpan={2} className="py-1.5 px-3 w-36 text-center align-middle font-black uppercase text-[9px] tracking-widest text-slate-400 whitespace-nowrap">Thành tiền</th>
                                                ...
                                                <tr className="border-t border-slate-200 dark:border-white/10 dark:border-slate-800/20 bg-transparent">
                                                    <td className="px-4 py-1 text-center border-r border-white/10 dark:border-slate-800/20 whitespace-nowrap">
                                                        <div className="flex items-center justify-center gap-1.5">
                                                            ...
                                                            <span className="text-[8px] font-black text-slate-400/80 uppercase tracking-widest">
                                                                Tổng đối soát ({totalItems} món)
                                                            </span>
                                                        </div>
                                                    </td>
                                                    ...
                                                    <td className="px-3 py-1 text-center border-r border-white/10 dark:border-slate-800/20 whitespace-nowrap">
                                                        <div className="flex items-center justify-center gap-1.5">
                                                            <span className="text-xs font-black text-slate-500 tabular-nums">{formatNumber(totalQty)}</span>
                                                        </div>
                                                    </td>
                                                </tr>
                                            </thead>
                                                        ...
                                                        <div className="relative group/search">
                                                            <div className="relative">
                                                                ...
                                                                <input
                                                                    ...
                                                                    className="w-full py-2.5 px-2.5 pl-12 bg-transparent border border-border focus:border-primary focus:ring-4 focus:ring-primary/10 focus:bg-transparent rounded-xl font-black text-gray-800 dark:text-gray-100 uppercase outline-none transition-all shadow-none leading-relaxed placeholder:normal-case placeholder:leading-relaxed"
                                                                    autoComplete="off"
                                                                    value={searchTerm}
                                                                    onChange={(e) => {
                                                                    ...
                                                                    >
                                                                        <div className="max-h-[400px] overflow-y-auto no-scrollbar scroll-smooth" ref={productDropdownRef}>
                                                                            {filteredProducts.map((p, idx) => (
                                                                                ...
                                                                                >
                                                                                    <div>
                                                                                        <div className={cn("font-black uppercase tracking-tight flex items-center gap-2 leading-relaxed", idx === activeIndex ? "text-white" : "text-gray-800 dark:text-gray-100")}>
                                                                                            <span className="leading-relaxed transition-all duration-300" style={{
                                                                                                paddingLeft: idx === activeIndex ? '12px' : '0px'
                                                                                            }}>{p.name}</span>
                                                                                            {p.code && (
                                                                                                <span className={cn(
                                                                                                    "text-[9px] px-1.5 py-0.5 rounded font-black tabular-nums border",
                                                            ...
                                                            {workingItem.product && (
                                                                <div
                                                                    className="absolute bottom-full left-1/2 -translate-x-1/2 mb-3 p-1
                                                                                bg-white/85 dark:bg-slate-900/85 backdrop-blur-[32px] rounded-2xl border border-slate-200 dark:border-white/10 shadow-2xl
                                                                ...
                                                                >
                                                                    <div className="flex flex-col items-center px-4 py-2 hover:bg-black/5 dark:hover:bg-white/5 rounded-xl transition-colors">
                                                                        <span className="text-[9px] uppercase font-black text-slate-500/70 dark:text-slate-400 leading-none mb-1.5 tracking-[0.1em]">
                                                                            Vốn TB
                                                                        </span>
                                                                    ...
                                                                    <div className="w-[1px] bg-white/20 dark:bg-slate-800/50 mx-1 my-2" />
                                                                    <div className="flex flex-col items-center px-4 py-2 hover:bg-black/5 dark:hover:bg-white/5 rounded-xl transition-colors">
                                                                        <span className="text-[9px] uppercase font-black text-slate-500/70 dark:text-slate-400 leading-none mb-1.5 tracking-[0.1em]">
                                                                            Nhập cuối
                                                                        </span>
                                                                    ...
                                                                    <div className="absolute top-full left-1/2 -translate-x-1/2 border-[6px] border-transparent border-t-white/85 dark:border-t-slate-900/85" />
                                                                </div>
                                                            )}
                                                            <div className="relative w-full">
                                                                <input
                                                                    type="text"
                                                                    className="w-full p-2 text-right bg-transparent border-none focus:ring-0 rounded font-black outline-none text-lg tabular-nums dark:text-gray-200"
                                                ...
                                                <AnimatePresence initial={false}>
                                                    {historyLoading ? (
                                                        ...
                                                        cart.map((item, idx) => (
                                                            ...
                                                            >
                                                                <td className="py-2 px-4 text-center text-slate-400 font-black text-[11px] group-hover:text-primary transition-colors tabular-nums">{idx + 1}</td>
                                                                <td className="py-2 px-2 relative">
                                                                    ...
                                                                    >
                                                                        <input
                                                                            className={cn(
                                                                                "w-full h-auto py-2.5 px-4 bg-transparent border-none focus:ring-0",
                                                                                "text-[17px] font-black uppercase tracking-tight transition-all leading-relaxed placeholder:normal-case placeholder:leading-relaxed",
                                                                                "text-emerald-900 dark:text-emerald-300 placeholder:text-gray-300",
                                                                                item.ai_scanned && "pb-6",
                                                                                rowSearchIdx === idx
                                                                                    ? "bg-white/10 dark:bg-slate-800/30 shadow-xl rounded-xl"
                                                                                    : ""
                                                                            )}
                                                                            autoComplete="off"
                                                                        ...
                                                                        />
                                                                        {item.ai_scanned && (
                                                                            <div className="absolute left-4 bottom-1 flex items-center gap-1.5 z-10">
                                                                                {item.ai_matched_status === 'matched' ? (
                                                                                    <span className="px-1.5 py-0.5 rounded bg-emerald-500/10 dark:bg-emerald-500/20 text-emerald-600 dark:text-emerald-400 text-[9px] font-black flex items-center gap-1 border border-emerald-500/20">
                                                                                        <Sparkles size={10} className="text-emerald-500 dark:text-emerald-400" />
                                                                                        AI Tự khớp: "{item.ai_original_name}"
                                                                                        <button 
                                                                                            type="button" 
                                                                                            onClick={(e) => {
                                                                                                e.stopPropagation();
                                                                                    ...
                                                                                    <span className="px-1.5 py-0.5 rounded bg-amber-500/10 dark:bg-amber-500/20 text-amber-600 dark:text-amber-400 text-[9px] font-black flex items-center gap-1 border border-amber-500/20">
                                                                                        <AlertCircle size={10} className="text-amber-500 dark:text-amber-400" />
                                                                                        AI không khớp được: "{item.ai_original_name}"
                                                                                        <button 
                                                                                            type="button" 
                                                                                            onClick={(e) => {
                                                                                                e.stopPropagation();
                                                                                ...
                                                                                <div ref={rowSearchDropdownRef} className="max-h-64 overflow-y-auto no-scrollbar">
                                                                                    {products.filter(p => {
                                                                                        ...
                                                                                        .slice(0, 50).map((p, pIdx) => (
                                                                                            ...
                                                                                            >
                                                                                                <div>
                                                                                                    <div className={cn("font-black uppercase tracking-tight flex items-center gap-2 leading-relaxed", pIdx === rowActiveIndex ? "text-white" : "text-gray-800 dark:text-gray-100")}>
                                                                                                        <span className="leading-relaxed transition-all duration-300" style={{
                                                                                                            paddingLeft: pIdx === rowActiveIndex ? '12px' : '0px'
                                                                                                        }}>{p.name}</span>
                                                                                                        {p.code && (
                                                                                                            <span className="text-[9px] px-1.5 py-0.5 rounded font-black tabular-nums border bg-transparent text-gray-400 border-gray-200 dark:border-slate-700">
                                                                                                                {p.code}
                                                                                                            </span>
                                                                                                        )}
                                                                                                    </div>
                                                                ...
                                                                <td className="py-2 px-4 text-center text-slate-500 font-bold text-xs">
                                                                    ...
                                                                    <div className="flex flex-col items-center gap-1 group/price relative group-hover/price:z-[500]">
                                                                        ...
                                                                        <div className="relative w-full">
                                                                            <input
                                                                                type="text"
                                                                                className={cn(
                                                                                    "w-full p-2 text-right bg-transparent border-none focus:ring-0 rounded font-black outline-none text-lg tabular-nums",
         ...
         >
            ...
            >
                                        ...
                                        <div className="flex flex-col gap-3 relative z-10 flex-1 overflow-y-auto -mr-2 pr-2 pb-2">
                                            ...
                                            <div className="flex-1 space-y-3 pt-1">
                                                <div className={cn(
                                                    ...
                                                    <div className="text-[10px] font-black text-white/60 uppercase tracking-[0.3em] mb-1 text-center relative z-10">Tổng tiền nhập kho</div>
                                                    ...
                                                    <div className="flex flex-col gap-3">
                                                        ...
                                                        {paymentMethod === 'Transfer' && (
                                                            <div className="relative">
                                                                ...
                                                                <select
                                                                    className="w-full p-2.5 pl-24 pos-card border-blue-200 dark:border-blue-900/30 rounded-[1.2rem] font-bold text-xs outline-none focus:ring-2 focus:ring-blue-500/20 transition-all dark:text-white"
                        ...
                        <Portal>
                            <div className="fixed inset-0 z-[2000] flex justify-end font-sans">
                                ...
                                <m.div
                                    ...
                                    className="relative w-[340px] md:w-[400px] bg-transparent shadow-2xl h-full flex flex-col p-8 border-l border-white/10"
                                >
                                    <div className="flex justify-between items-center mb-8">
                                        ...
                                        <button
                                            onClick={() => setIsHeldSidebarOpen(false)}
                                            className="p-2 hover:bg-transparent dark:hover:bg-slate-800 rounded-full transition-all hover:rotate-90"
                                        >
                                            ...
                                            <AnimatePresence mode="popLayout">
                                                {heldPurchases.map((held, idx) => (
                                                    <m.div
                                                        ...
                                                        className="bg-transparent border-2 border-amber-50 dark:border-amber-900/10 rounded-[2rem] p-5 hover:border-amber-200 dark:hover:border-amber-600/30 transition-all group shadow-sm hover:shadow-xl hover:-translate-y-1"
                                                    >
                                                        <div className="flex justify-between items-start mb-4">
                                                            <div className="flex-1">
                                                                ...
                                                                <div className="flex items-center gap-3 mt-2">
                                                                    <div className="text-[10px] font-black text-gray-400 bg-transparent px-2 py-0.5 rounded-full uppercase tabular-nums">
                                                                        🕒 {held.time}
                                                                    </div>
                                                        ...
                                                        <div className="flex justify-between items-center bg-transparent p-3 rounded-2xl border border-dashed border-gray-200 dark:border-slate-700">
                                                            <div>
                                    ...
                                    <div className="mt-8 pt-6 border-t border-gray-100 dark:border-slate-800">
                                        <button
                                            onClick={() => setIsHeldSidebarOpen(false)}
                                            className="w-full py-4 rounded-2xl border-2 border-gray-200 dark:border-slate-800 text-gray-500 font-black uppercase text-xs hover:bg-transparent dark:hover:bg-slate-800 transition-all tracking-widest"
                                        >
                                            Đóng sidebar
                                        </button>
                                    </div>
                                    ...
                                    >
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div className="flex flex-col min-w-0">
                                                <h3 className="text-base font-bold text-foreground uppercase tracking-wide leading-relaxed py-1 truncate">
                                                    Quét Hóa Đơn Bằng AI
                                                </h3>
                                                <p className="text-muted-foreground text-[10px] font-medium uppercase tracking-widest mt-0.5">Tự động nhận dạng & thêm vào giỏ bằng Gemini</p>
                                            </div>
                                        </div>
                                        <button 
                                            onClick={() => {
                                            ...
                                            className="w-8 h-8 flex items-center justify-center rounded-lg bg-transparent hover:bg-muted/50 text-muted-foreground hover:text-foreground transition-colors"
                                        >
                                    ...
                                    <div className="p-6 overflow-y-auto flex-1 space-y-6 custom-scrollbar">
                                        ...
                                        {!settings.gemini_api_key && (
                                            <div className="p-4 bg-amber-500/10 border border-amber-500/20 dark:border-amber-500/30 rounded-2xl space-y-2">
                                                <div className="flex items-center gap-2 text-amber-650 dark:text-amber-400 font-bold text-xs uppercase">
                                                ...
                                                <input
                                                    ...
                                                    className="w-full p-3 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-xs font-mono outline-none focus:border-primary text-slate-900 dark:text-white focus:ring-1 focus:ring-primary/35 transition-all"
                                                />
                                            </div>
                                        )}

                                        <div className="space-y-4 flex flex-col">
                                            <div className="flex-1 flex flex-col justify-center items-center border-2 border-dashed border-slate-200 dark:border-slate-700/80 hover:border-primary/50 rounded-[2rem] p-6 bg-slate-50/50 dark:bg-slate-800/20 min-h-[260px] relative overflow-hidden group transition-all duration-300">
                                                {scannedImages.length > 0 ? (
                                                    <div className="w-full h-full flex flex-col space-y-4">
                                                        ...
                                                        <div className="flex justify-center gap-3">
                                                            <label 
                                                                ...
                                                                className="px-5 py-2.5 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 font-bold text-xs rounded-xl cursor-pointer transition-all uppercase tracking-wider flex items-center gap-1.5 active:scale-95"
                                                            >
                                                            ...
                                                            <button 
                                                                ...
                                                                className="px-5 py-2.5 bg-rose-500/10 hover:bg-rose-500/20 text-rose-600 dark:text-rose-450 font-bold text-xs rounded-xl transition-all uppercase tracking-wider active:scale-95"
                                                            >
                                                                Xóa tất cả
                                                            </button>
                                                        </div>
                                                    ...
                                                    >
                                                        <div className="p-5 bg-primary/10 text-primary rounded-full group-hover:scale-110 group-hover:bg-primary/20 transition-all duration-300">
                                                            <Camera size={36} />
                                                        </div>
                                            ...
                                            {scannedImages.length > 0 && (
                                                <m.button
                                                    ...
                                                    className="w-full p-4 bg-gradient-to-r from-emerald-600 to-teal-650 hover:brightness-110 disabled:opacity-50 text-white font-black uppercase text-xs tracking-wider rounded-2xl flex items-center justify-center gap-2 shadow-lg active:scale-[0.98] transition-all"
                                                >
                                                    {isScanning ? (
                                                        <>
                        ...
                        <Portal>
                            <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-300 overflow-y-auto">
                                <m.div
                                    ...
                                    className="bg-transparent rounded-3xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col my-auto relative z-10"
                                >
                                    <div className="p-6 border-b dark:border-slate-800 flex justify-between items-center bg-transparent">
                                        ...
                                        <div className="flex items-center gap-2">
                                            ...
                                            <button
                                                ...
                                                className="p-2.5 bg-transparent text-slate-400 hover:text-red-500 rounded-xl transition-all"
                                            >
                                                <X size={24} />
                                            </button>
                                        </div>
                                    ...
                                    <div className="flex-1 overflow-y-auto p-8 bg-transparent dark:bg-slate-950 flex justify-center custom-scrollbar">
                                        <div className="bg-transparent shadow-2xl h-fit">
                                            <PrintTemplate
                                                data={previewData}
                                                settings={settings}
                                                type="Purchase"
                ...
                <PurchaseHistoryPanel
                    ...
                    onAddToCart={(p) => addToCart(p, 1, p.cost_price || 0)}
                    onEditOrder={(order) => {
                        loadOrder(order);
                        setIsHistoryPanelOpen(false);
                        setToast({ message: 'Đã mang đơn nhập ra màn hình để sửa!', type: 'success' });
                    }}
