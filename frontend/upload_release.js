...
const REPO_OWNER = process.env.GH_REPO_OWNER || 'luongnghia6799';
const REPO_NAME = process.env.GH_REPO_NAME || 'lyangpos_thi';
...
function getGitHubToken() {
  ...
  // Thu doc tu .env hoac .env.local
  const envFiles = ['.env', '.env.local', '../.env'];
  ...
  // Thu lay tu GitHub CLI (gh auth token)
  try {
    const ghCmd = fs.existsSync('C:\\Program Files\\GitHub CLI\\gh.exe') ? '"C:\\Program Files\\GitHub CLI\\gh.exe"' : 'gh';
    const ghToken = execSync(`${ghCmd} auth token`, { encoding: 'utf-8', stdio: ['pipe', 'pipe', 'ignore'] }).trim();
    if (ghToken) return ghToken;
  } catch (e) {
    // ignore
  }

  // Thu lay tu git credential fill
  try {
    const gitCmd = fs.existsSync('C:\\Program Files\\Git\\cmd\\git.exe') ? '"C:\\Program Files\\Git\\cmd\\git.exe"' : 'git';
    const cred = execSync(`${gitCmd} credential fill`, {
...
async function uploadRelease() {
  ...
  // Uu tien dung GitHub CLI (gh) neu co
  let uploadedViaGh = false;
  try {
    const ghCmd = fs.existsSync('C:\\Program Files\\GitHub CLI\\gh.exe') ? '"C:\\Program Files\\GitHub CLI\\gh.exe"' : 'gh';
    execSync(`${ghCmd} release upload ${tagName} "${assetFilePath}" --clobber --repo ${REPO_OWNER}/${REPO_NAME}`, {
      stdio: 'inherit'
    });
    uploadedViaGh = true;
  } catch (err) {
    // Neu gh that bai hoac khong co, dung REST API
  }

  if (!uploadedViaGh) {
    const fileBuffer = fs.readFileSync(assetFilePath);
    let uploadBaseUrl = release.upload_url ? release.upload_url.split('{')[0] : `https://uploads.github.com/repos/${REPO_OWNER}/${REPO_NAME}/releases/${release.id}/assets`;
    const uploadUrl = `${uploadBaseUrl}?name=${encodeURIComponent(assetFileName)}`;
  ...
  console.log(`[>] Direct Download: https://github.com/${REPO_OWNER}/${REPO_NAME}/releases/download/${tagName}/${encodeURIComponent(assetFileName)}`);
  console.log('=====================================================================\n');
}
